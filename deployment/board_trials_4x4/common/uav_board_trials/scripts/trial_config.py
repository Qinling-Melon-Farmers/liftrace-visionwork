...
def generate(root,out,settings,fc_xyz,rig):
    ...
    runtime['high_view_probe']['low_stage_parameters']=[dict(name=key,value=value) for key,value in {'/external_planner_max_command_z':ground+1.85,'/fast_planner_node/sdf_map/virtual_ceil_height':(ground+2. if ceiling_enabled else -.1),'/fast_planner_node/fsm/goal_adjustment_radius':.15}.items()]
    runtime['high_view_full']=dict(policy=dict(high_max_agl=3.0,candidate_min_streak=1,min_interval_ns=100000000,min_span_ns=200000000,max_uncertainty_m=.45,direct_descent=True,descent_radius_m=1.,descent_max_candidates=9,survey_stall_seconds=8.,survey_progress_m=.15,survey_alternative_radius_m=.3),grid=dict(bounds=bounds,resolution=.10,inflation=.25),boundary_policy=dict(enabled=True,bounds=[x-.35,x+4.,y-2.,y+2.]))
    ...
    # Keep measured body slot geometry from the current flight team's rig.
    ...
    overrides={
        ...
        '/fast_planner_node/sdf_map/horizontal_avoidance/enabled':mode=='high_view',
