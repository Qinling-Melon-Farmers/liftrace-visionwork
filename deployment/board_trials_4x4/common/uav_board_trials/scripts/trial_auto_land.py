...
def trial_ready(status,context,frame):
    return (bool(context) and context.get('scope')=='board_trial_landing_after_mock'
            ...
            and context.get('decision_seq')==status.get('active_decision_seq'))


def route_finished(status, revision):
    return (status.get('start_mode') in ('full', 'post_delivery')
...
class AutoLand:
    def __init__(self):
        ...
        self.context = None
        self.handoff_pub=rospy.Publisher('/board_trials/auto_land_status',String,queue_size=1,latch=True)
    ...
    def on_context(self,msg):
        with self.lock:
            try:
                value=json.loads(msg.data);xy=value['xy'];z=float(value['z'])
                if len(xy)!=2 or not all(math.isfinite(v) for v in list(xy)+[z]) or not -.05<=rospy.Time.now().to_sec()-value['time']<=.5:raise ValueError('invalid/stale landing context')
                self.context=value;self.xy=list(xy);self.z=z
            except (ValueError,TypeError,KeyError):self.context=None
    ...
    def tick(self, _event):
        with self.lock:
            ...
            valid = (trial_ready(self.status,self.context,self.frame)
            ...
            try:
                ...
                self.handoff_pub.publish(String(data=json.dumps(dict(mode_sent=self.handed_over,mission_id=self.context['mission_id'],decision_seq=self.context['decision_seq'],xy=self.xy,z=self.z,time=now))))
                rospy.logwarn('Board trial AUTO.LAND request %d accepted=%s; check MAVROS mode/landed state',
                              self.attempts, self.handed_over)
            except rospy.ServiceException as error:
                rospy.logerr('Board trial AUTO.LAND request failed: %s', error)


if __name__ == '__main__':
    rospy.init_node('trial_auto_land')
    AutoLand()
    rospy.spin()
