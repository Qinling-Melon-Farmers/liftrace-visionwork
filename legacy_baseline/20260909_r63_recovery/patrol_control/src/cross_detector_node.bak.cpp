...
CrossDetectorNode::CrossDetectorNode(ros::NodeHandle nh)
    ...
    ROS_INFO("\033[32m[CrossDetectorNode] Publishing binary result image to: /detect/cross_binary_result_image/compressed\033[0m");
}
...
void CrossDetectorNode::loadParameters() {
    ...
    nh_.param("red_cross_detection/h_max", red_h_max_, 5);
...
void CrossDetectorNode::imageCallback(const sensor_msgs::ImageConstPtr& msg) {
    ...
    try {
        ...
        bool red_cross_detected = detectRedCross(processed_image, cross_center, cross_area, red_contours, red_mask, quality_params);
        ...
        if (red_cross_detected) {
            ...
            pixel_offset.x = -pixel_offset_x * pixel_offset_scale_;
            pixel_offset.y = -pixel_offset_y * pixel_offset_scale_;
            pixel_offset.z = sqrt(cross_area / M_PI) * 4.34; // 使用面积计算等效半径

            double waypoint_x = -pixel_offset_y * pixel_offset_scale_;
            double waypoint_y = -pixel_offset_x * pixel_offset_scale_;
            double waypoint_z = pixel_offset.z;
            ...
            ROS_INFO_THROTTLE(0.5, "\033[35m[CircleDetectorNode]   - Circle Center: (%.1f, %.1f)\033[0m", cross_center.x, cross_center.y);
            ...
            ROS_INFO_THROTTLE(0.5, "\033[35m[CircleDetectorNode]   - Scale Factor: %.6f\033[0m", pixel_offset_scale_);
            ROS_INFO_THROTTLE(0.5, "\033[35m[CircleDetectorNode]   - Circle Radius: %.1f\033[0m", pixel_offset.z);
            ...
            ROS_INFO_THROTTLE(0.5, "\033[35m[CircleDetectorNode]   - waypoint_x = -%.1f * %.6f = %.6f\033[0m", pixel_offset_y, pixel_offset_scale_, waypoint_x);
            ROS_INFO_THROTTLE(0.5, "\033[35m[CircleDetectorNode]   - waypoint_y = -pixel_offset_x * scale_factor_\033[0m");
            ROS_INFO_THROTTLE(0.5, "\033[35m[CircleDetectorNode]   - waypoint_y = -%.1f * %.6f = %.6f\033[0m", pixel_offset_x, pixel_offset_scale_, waypoint_y);
            ...
            pixel_offset_pub_.publish(waypoint_mark);
        ...
        // 绘制带质量参数标注的二值化图像
        cv::Mat binary_result_image = drawBinaryResultWithQualityParams(red_mask, red_contours, red_cross_detected ? &cross_center : nullptr, quality_params);
...
cv::Mat CrossDetectorNode::drawDetectionResult(const cv::Mat& image) {
...
cv::Mat CrossDetectorNode::drawBinaryResultWithQualityParams(const cv::Mat& red_mask, const std::vector<std::vector<cv::Point>>& contours, const cv::Point2f* best_cross_center, const std::vector<double>& quality_params) {
    // 将二值化图像转换为彩色图像
    cv::Mat binary_color;
    ...
    if (best_cross_center && !quality_params.empty()) {
        ...
        if (quality_params.size() > 1) {
            cv::putText(binary_color, cv::format("Contour Points: %d", (int)quality_params[1]), 
                        cv::Point(10, y_offset), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 255, 0), 2);
            y_offset += line_height;
        }
