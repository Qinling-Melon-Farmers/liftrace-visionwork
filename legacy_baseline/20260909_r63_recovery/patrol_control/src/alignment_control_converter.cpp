...
AlignmentControlConverter::AlignmentControlConverter(ros::NodeHandle& nh) : nh_(nh) {
    ...
    count1 = 1;
    servo_mark = false;
    servo_marky.data = false;
    servo_status.data = false;
    ...
    nh_.param<std::string>("odom_topic", odom_topic, "/mavros/local_position/odom");
    odom_sub_ = nh_.subscribe(odom_topic, 1, &AlignmentControlConverter::odomCallback, this);
    circle_detection_control_sub_ = nh_.subscribe("/detect/control", 1, &AlignmentControlConverter::CircleDetectionControlCallback, this);
    cross_detection_control_sub_ = nh_.subscribe("/detect/cross_control", 1, &AlignmentControlConverter::CrossDetectionControlCallback, this);
    landing_detection_control_sub_ = nh_.subscribe("/detect/landing_control", 1, &AlignmentControlConverter::detectionControlCallback, this);
...
void AlignmentControlConverter::CircleDetectionControlCallback(const std_msgs::Bool::ConstPtr& msg) {
    bool prev_state = circle_is_detection_active_;
    //ROS_INFO("is_detection");
    circle_is_detection_active_ = msg->data;
    
    // 只在状态变化时输出
    if (prev_state != circle_is_detection_active_) {
        if (circle_is_detection_active_) {
            ROS_INFO("\033[1;32m[AlignmentConverter] Circle Detection ENABLED - Listening for pixel offsets\033[0m");
            // 重置渐进降高状态
            descent_started_ = false;
            detection_count_ = 0;
            first_detection_ = true;
        } else {
            ROS_INFO("\033[1;33m[AlignmentConverter] Circle Detection DISABLED\033[0m");
        }
    }
}
void AlignmentControlConverter::CrossDetectionControlCallback(const std_msgs::Bool::ConstPtr& msg) {
    bool prev_state = cross_is_detection_active_;
    //ROS_INFO("is_detection");
    cross_is_detection_active_ = msg->data;
    
    // 只在状态变化时输出
    if (prev_state != cross_is_detection_active_) {
        if (cross_is_detection_active_) {
            ROS_INFO("\033[1;32m[AlignmentConverter] Circle Detection ENABLED - Listening for pixel offsets\033[0m");
            // 重置渐进降高状态
            descent_started_ = false;
            detection_count_ = 0;
            ...
            ROS_INFO("\033[1;33m[AlignmentConverter] Cross Detection DISABLED\033[0m");
        }
    }
}
void AlignmentControlConverter::detectionControlCallback(const std_msgs::Bool::ConstPtr& msg) {
    bool prev_state = land_is_detection_active_;
    land_is_detection_active_ = msg->data;
    
    // 只在状态变化时输出
    if (prev_state != land_is_detection_active_) {
        if (land_is_detection_active_) {
            ROS_INFO("\033[1;32m[AlignmentConverter] Land Detection ENABLED - Listening for pixel offsets\033[0m");
            // 重置渐进降高状态
            descent_started_ = false;
            detection_count_ = 0;
            first_detection_ = true;
...
void AlignmentControlConverter::pixelOffsetCallback(const geometry_msgs::Point::ConstPtr& msg) {
    ...
    if (!circle_is_detection_active_ && !land_is_detection_active_ && !cross_is_detection_active_) {
    ...
    DetectionType flag = DetectionType::CIRCLE;  // 默认值
    if(circle_is_detection_active_){
        flag = DetectionType::CIRCLE;
    }
    else if(land_is_detection_active_){
        flag = DetectionType::LANDING;
    }
    else if(cross_is_detection_active_){   
        flag = DetectionType::CROSS;
    }
    ...
    double real_circle_radius = 0.3;  // 米
    if(!land_is_detection_active_) real_circle_radius = 0.5;
    ...
    ROS_INFO("pixel_error %f",meter_error_x);
    ROS_INFO("y %f",meter_error_y);
    // 获取无人机当前位置和姿态
    ...
    if (progressive_descent_enable_) {
        ROS_INFO("GO ON 3");
        // 第一次检测到圆环，记录初始高度
        ROS_INFO("first_detection_: %s", first_detection_ ? "true" : "false");
        bool a = !first_detection_ && cross_is_detection_active_;
        ROS_INFO("a: %s", a ? "true" : "false");
        if (first_detection_ || (!first_detection_ && cross_is_detection_active_)) {
            initial_height_ = uav_z;
            first_detection_ = false;
            servo_mark = false;  // 重置舵机标志
            ROS_INFO("\033[1;34m[AlignmentConverter] First circle detection! Initial height: %.2f m\033[0m", initial_height_);
        }
        std::cout<<"detection_count_ >= progressive_descent_min_detection_count_ = "<<detection_count_<<" >= "<<progressive_descent_min_detection_count_<<std::endl;
        // 检测次数达到阈值后开始渐进降高
        ...
        std::cout<<"descent_started_ = "<<descent_started_<<std::endl;
        // 如果已开始渐进降高，计算当前目标高度
        if (descent_started_) {
            ros::Time current_time = ros::Time::now();
            ...
            if (elapsed_time <= progressive_descent_duration_) {
                ...
                target_z = initial_height_ + (land_height_[static_cast<int>(flag)] - initial_height_) * progress;
                ...
                servo_mark = true;
                target_z = 0.05;
                ROS_INFO("\033[1;33m[AlignmentConverter] Descent completed, servo mark set to true\033[0m");
            }
        }
    }
    ...
    double target_x = 0;
    double target_y = 0;
    ROS_INFO("[AlignmentConverter] servo_mark: %d, uav_z: %.3f", servo_mark, uav_z);
    if(!servo_mark){
        ROS_INFO("[AlignmentConverter] servo_mark: offset mode - using pixel offset");
        temp_mark = 0;
        ...
        if(temp_mark == 0 && count1 == 1 && !land_is_detection_active_){
            temp_x -= 0.15;
            temp_mark = 1;
            count1++;
            current_time_ = ros::Time::now();
            ROS_INFO("[AlignmentConverter] count1: 1 - temp_x: %.2f", temp_x);
        }
        else if(temp_mark == 0 && count1 == 2 && !land_is_detection_active_){
            temp_y -= 0.1;
            temp_mark = 1;
            count1++;
            current_time_ = ros::Time::now();
            ROS_INFO("[AlignmentConverter] count1: 2 - temp_y: %.2f", temp_y);
        }
        else if(temp_mark == 0 && count1 == 3 && !land_is_detection_active_){
            temp_y += 0.1;
            temp_mark = 1;
            count1 = 1;  // 重置为1而不是0
            current_time_ = ros::Time::now();
            ROS_INFO("[AlignmentConverter] count1: 3 - temp_y: %.2f", temp_y);
        }
        target_x = temp_x;
        target_y = temp_y;
    }
