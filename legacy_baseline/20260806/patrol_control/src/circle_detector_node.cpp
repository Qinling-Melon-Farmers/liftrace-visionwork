...
public:
    CircleDetection(const ros::NodeHandle &nh) : it(nh), transform_listener(buffer) {
        ...
        this->point_class_sub_ = node.subscribe("/detect/point_class",1,&CircleDetection::pointclass_callback,this);
    ...
    void pointclass_callback(const std_msgs::Int8& msg)
    {
        this->pointclass = msg.data;
        switch (pointclass)
        {
        case 4:
            {
                dynamic_calibration = 0.25;
            }
            default:
            {
                dynamic_calibration = 0.0;
            }
        }
    }
