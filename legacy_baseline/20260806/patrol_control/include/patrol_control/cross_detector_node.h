...
private:
    ...
    cv::Mat drawDetectionResult(const cv::Mat& image);  // 修改返回类型为cv::Mat
    cv::Mat drawBinaryResultWithQualityParams(const cv::Mat& red_mask, const std::vector<std::vector<cv::Point>>& contours, const cv::Point2f* best_cross_center, const std::vector<double>& quality_params);
