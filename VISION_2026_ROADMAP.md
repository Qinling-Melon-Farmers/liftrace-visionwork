...
13. 控制组提出斜下辅助相机提前发现需求；`V-EXP-01` 已在独立 feature 分支落下隔离原型、
    ...
    不可达而 FAIL，稳定性尚未通过。推荐架构调整为“一个常驻 YOLO/RKNN 按任务阶段切换
    斜下/下视输入”，不要求双模型常开；下视链仍是类别、精地图点和投递证据唯一权威。
    下一 Gate 必须使用五类标准靶与红十字全部随机布设的成对多 seed，覆盖辅助视场
    0/部分/全部可见和红十字高权重中断，才能支持导航接入或硬件决策。
...
| 原型完成/稳定性未过 | 16 | V-EXP-01 | 斜下辅助相机搜索可行性 | 保持下视权威链；固定坐标主动 A/B 已证明单轮大收益但暴露规划不可达。下一 Gate 采用单 YOLO 双相机阶段切源，并在全随机全靶标、辅助 0/部分/全部可见和红十字场景中按最终发现率、净搜索时间、净路径、fallback/不可达决定接入或放弃 |
