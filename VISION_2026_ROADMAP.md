...
13. 控制组提出斜下辅助相机提前发现需求；`V-EXP-01` 已在独立 feature 分支落下隔离原型、
    单目/深度粗投影、运行期派生机架、固定矩阵和 coverage shadow 入口。L0 与 55°/2 m/tent
    单目标烟测通过，但完整 126-case 和覆盖航线 Gate 未跑，因此不改变“下视链是唯一权威
    投递链”的当前架构，也尚不能支持采购或导航接入决策。
...
| 原型完成/量化待跑 | 16 | V-EXP-01 | 斜下辅助相机搜索可行性 | 不改下视权威链；完成 mono/depth 固定 126-case 与 coverage shadow，按提前发现、P90 粗地图误差和模拟交接 Gate 决定单目/双目/放弃 |
