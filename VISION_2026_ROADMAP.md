...
5. `V-CL-04`：靶标区域的非靶标坐标覆盖导航已完成；下一 Gate 验证权重候选队列、三次
   投放、恢复、返航和降落。

当前主线进入 `V-CL-04` 完整闭环：只验证靶标区域的候选队列、超时恢复、权重三投和
返航降落；北区走廊避障按用户要求后置，不计入本轮 R6 Gate。
...
| 部分完成/三投待验收 | 14 | V-CL-04 | 覆盖搜索、候选队列和恢复 | `target_area_navigation_20260807_190817` 已完成靶标区域 12/12 覆盖、五类五 ID 和返航；下一步执行权重三投 R6 |
