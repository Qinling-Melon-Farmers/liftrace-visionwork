...
7. `V-CL-06`：迁移到导航组搜索 manager。保持上游源码不变，由外围适配器接入起飞门控、
   ...
   下一完成定义是双方先冻结“实时中断候选/持久地图队列/投递结果”接口，再由导航组任务
   层实现全局权重、失败冷却重试和剩余时间调度；同一 headless Gate 在 600 s 内完成三投、
   返航和落地。视觉组负责候选质量和新鲜度，不在 `uav_vision` 内实现任务队列。
...
| 接口已接入/Gate FAIL | 16 | V-CL-06 | 导航组 manager + 新视觉正式任务链接入 | 上游 manager/策略保持原逻辑，raw/planner goal 所有权正确；首轮 600 s 为 9/16 覆盖、pillbox/tent/bridge 三候选、tent/bridge 2/3 投递、mission timeout。下一步联合冻结队列/结果接口，由导航组实现持久全局权重、失败重试和时间调度；同 seed 600 s 三投+返航+落地才 PASS |
