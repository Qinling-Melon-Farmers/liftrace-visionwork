> 当前本轮任务已停止：十seed0组，R58最后实跑FAIL；见[收口报告](docs/verification/r58_closeout/REPORT.md)。旧条目为历史，不自动续跑。
...
19. V-CL-06 的接口和视觉执行面已随 PR #3 合入视觉主线；导航上游 PR #1～#6 也已合入
    `main@a182ca9`，当前完整任务入口为上游
    `feat/vcl06-local-full-mission@c1be1d0`，原跨 fork PR #8 已按 owner 安排关闭为 superseded。
    r11 实际导航代码 `919e438` 在正式 seed=11 随机场完成 panzer、bridge、red_cross 三个
    selected→APPROACH→CAPTURE→release→recovery 事务，三槽 3/3、视觉投递审计 PASS；
    `/navigation/planner_bridge` 是唯一 goal 发布者，unmatched result、tank selected/accepted、
    碰撞、越界和高度违规均为 0，最高高度 `3.147812 m`。三投后进入 11 点路线并通过 Wall_15，
    但 route index 2 在距 Fast-Planner 调整后 effective goal 约 0.13 m 时反复进入 `<0.2 m`
    `Close goal`，执行桥又等待 `TRAJECTORY_FINISHED`，最终
    `planner_attempt_limit_exceeded→return_home_failed`；整场 Gate 仍为 FAIL，Wall_20、Wall_22、
    H 与 LAND 尚未触达。r11 已关闭 r9 的目标接收/替换竞态与 4 m 超高，也足以关闭 V-CL-05
    的随机场前三权重中断/三投子 Gate；下一阻塞明确属于导航近目标终态，不回填到视觉 adapter。
20. 顶层 20 项任务按严格状态统计为 **10 项闭合、9 项开放、1 项冻结**。开放项中严格视觉
    主责为 5 项；若把 H 结构/降落联合 Gate 计入视觉参与面，则为 6 项。其余开放项主要是
    V-CL-00B/01/06 的联合接线、导航执行和整机验收，不能计成视觉算法欠账。
...
22. 2026-09-05 r41 已在视觉 `8e53bd0`、导航 `3557215`、KS2A543、seed=11 上完成正式
    ...
    Phase D 默认关闭 legacy bridge；视觉远端 main 尚缺该候选的 30 个运行提交。导航运行候选
    `be85c42` 已在 `6c59e17` 计时修订之上完成 typed H、bridge 关闭和视觉副本删除并 clean
    推送；远端 branch tip `b74b918` 只追加导航 Handoff，整个分支领先上游 main 41 个提交。最终
    先对精确双仓运行代码组合做 clean 静态复核，再生成一次持久化完整 Gate 证据，最后由人工
    合并和开展板端整机资源验收。详细顺序见
...
23. 2026-09-05 部署前复核以视觉组已完成的 worktree 清理为新基线：当前 WSL 工程树和既有
    ...
    不新增消息或兼容模式。视觉侧默认旧话题和无效订阅已在 `9324745` 清理，导航侧已在
    `be85c42` 提交对应减法；最终双仓 clean 联合静态验证仍待执行。轻量策略替换、MJPEG/运动
    模糊专项继续后移，不与最终唯一整场 Gate 混跑。本轮又移除了两个可重建视觉 checkout，
...
7. `V-CL-06`：r41 已在 seed=11 正式随机场完成三个不同 stable ID 的 APPROACH、CAPTURE、
   ...
   视觉 `9324745` 已合入外参并默认关闭 legacy bridge；导航运行代码 `be85c42` 已提交 typed H、
   关闭 bridge 和视觉副本删除，`b74b918` 仅为其 Handoff tip。下一完成定义是联合静态验证单一
   源码/话题/权威节点，再在最终 revision 上只跑一轮交付 Gate；不在视觉侧复制策略。
   `/detect/point_class` 暂留为 release arbiter 和
   Planner Bridge 的有效控制状态，不为改名新增别名。baseline/a68925d 历史 A/B 仍不满足推广条件。
...
| 功能闭环通过/接口减法待最终 Gate | 16 | V-CL-06 | 导航组 manager + 新视觉正式任务链接入 | r41 使用视觉 `8e53bd0`、导航 `3557215` 和 KS2A543 完成三投、三门、H、LAND，任务时钟 429.875 s；历史形式 FAIL 仅为误用 1761.101 s 主机墙钟。视觉运行候选 `9324745` 与导航 `be85c42` 已分别 clean 推送外参、typed H、关闭 bridge 和单一视觉源码；待最终组合 clean 静态验证并只跑一次最终 Gate |
