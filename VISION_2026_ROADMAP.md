...
13. 控制组提出斜下辅助相机提前发现需求；`V-EXP-01` 已在独立 feature 分支落下隔离原型、
    单目/深度粗投影、OpenCV 蓝环低资源链和评测专用主动导航外挂。固定坐标第 1 组 A/B
    中辅助搜索时间缩短 47.5%、路径缩短 40.2%，但第 2 次辅助运行因候选抵近/返航规划
    目标无进展而 FAIL，稳定性尚未通过。2026-08-20 已补候选五态生命周期和辅助→下视
    新鲜空间交接：强制 2 候选轮实现 2/2 交接（0.110/0.313 m），fallback 后找齐五类，
    但返航遇 Fast-Planner 内存/重规划失败并撞墙钟。推荐架构仍为“一个常驻 YOLO/RKNN
    按任务阶段切换斜下/下视输入”，不要求双模型常开；下视链仍是类别、精地图点和投递
    证据唯一权威。下一步先完成固定世界多轮触发策略稳定性，再依次做 proposal 抽象、
    辅助 YOLO/单 YOLO 切源和全随机全靶标 0/部分/全部可见 Gate。
...
5. `V-CL-04`：2026-08-13 实跑（`logs/toudi4_coverage_r6_v2_20260813_221737/`）
   ...
   累计事实修复；下一次干净复跑确认 Gate PASS（Fast-Planner 可达性波动为规划侧
   已知问题，不阻塞视觉闭环验收）。H 视觉降落 Gate 另立；北区走廊独立成 Gate。
...
| Step 1 交接成立/稳定性未过 | 16 | V-EXP-01 | 斜下辅助相机搜索可行性 | 保持下视权威链；候选五态与新鲜空间交接已实现，强制轮 2/2 交接且 fallback 找齐五类，返航规划失败使整轮 FAIL。先完成固定世界多轮触发策略，再按 `aux_camera_single_yolo_roadmap.md` 推进 proposal 抽象、辅助 YOLO、单 YOLO 切源和全随机全靶标 Gate |
