...
- 泛化圆环circle不等于装甲车panzer已确认；landing模式下仍可能有原始类别/圆环检测，却不会进入投递类别融合链。

## 验证范围

已用2026-09-25 109.10秒实飞bag端到端导出四种视频并检查；通用性测试覆盖时间匹配、静态坐标旋转/逆变换、未知TF及动态过期。尚未在队友不同发行版、ROS2或只有原始Image的bag上验收。
