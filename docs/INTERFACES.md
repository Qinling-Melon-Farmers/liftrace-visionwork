...
2026-09-12研究接入约定：新增默认关闭、仅允许仿真时钟/full任务模式的局部入口执行开关。Mission Manager消费schema=1的局部建议及最新观测generation，独占接纳决策；建议本身仍不是飞行授权。每个名义航点最多插入一个入口、每场默认最多四次，单次最多12s，并保留原航点deadline。入口终止不完成名义航点、不增加名义失败次数；过期原deadline仍按原失败流程处理，高权重目标/返航优先。新decision_seq经既有Planner Bridge接管，旧回执不推进游标；本阶段只执行entry，不预执行建议中的exit，收益使用独立entry区域估计。
