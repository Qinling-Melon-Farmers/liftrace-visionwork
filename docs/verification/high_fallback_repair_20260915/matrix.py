"""Explicitly authorized, serial three-run driver. No automatic retries."""
import argparse,json,os,subprocess,time,shutil,fcntl
from pathlib import Path
D=Path(__file__).resolve().parent;R=D.parents[2]
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--execute-authorized-three',action='store_true');args=parser.parse_args()
    if not args.execute_authorized_three:raise SystemExit('Execution requires explicit current-request authorization flag')
    lock=open('/tmp/liftrace_high_fallback_repair.lock','w');fcntl.flock(lock,fcntl.LOCK_EX|fcntl.LOCK_NB)
    batch=R/'logs/high_fallback_repair_20260915_batch';batch.mkdir(exist_ok=True)
    ...
    for case in json.loads((D/'cases.json').read_text()):
        seed=case['seed'];prefix=f'high_fallback_repair_seed{seed}';before=set((R/'logs').glob(prefix+'_*'))
    ...
    state['status']='COMPLETE';save();print('Three runs finished; no retries.',flush=True)
