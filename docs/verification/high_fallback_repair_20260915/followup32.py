"""Explicitly authorized, serial single follow-up driver. No automatic retries."""
import argparse,json,os,subprocess,time,shutil,fcntl
from pathlib import Path
D=Path(__file__).resolve().parent;R=D.parents[2]
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--execute-authorized-followup',action='store_true');args=parser.parse_args()
    if not args.execute_authorized_followup:raise SystemExit('Execution requires explicit current-request authorization flag')
    lock=open('/tmp/liftrace_descent32_followup.lock','w');fcntl.flock(lock,fcntl.LOCK_EX|fcntl.LOCK_NB)
    batch=R/'logs/high_fallback_descent32_20260915_batch';batch.mkdir(exist_ok=True)
    ...
    for case in [c for c in json.loads((D/'cases.json').read_text()) if c['seed']==32]:
        seed=case['seed'];prefix=f'high_fallback_descent32_seed{seed}';before=set((R/'logs').glob(prefix+'_*'))
    ...
    state['status']='COMPLETE';save();print('Single follow-up finished; no retries.',flush=True)
