...
另有配置变化：r41 横向膨胀 0.20 m、门前/门后 0.90 m、高空过渡 2.20 m、控制命令距离限制 0.20 m；当前分别为 0.30 m、0.75 m、1.40 m 分段降高、0.40 m。当前还使用正装机顶 MID360、相机低于 IMU 21 cm、飞控 IMU/气压计高度融合，不能把旧场景 PASS 直接移到新组合。
...
这说明“保留包络、提高走廊地图精度”有具体依据，同时证明照抄旧 0.9 m 并不必然可用。上述只检查有限位置的占据，未覆盖持续地图缓冲、完整 ESDF、动态姿态和实体碰撞，不是穿门 PASS。
