...
def main():
    runs=sorted(p for p in (ROOT/'logs').glob('coverage_seed*') if (p/'coverage/status.jsonl').exists())
    rows=[analyze(run) for run in runs if (run/'scenario_inputs/runtime.yaml').exists()]
    (OUT/'run_metrics.json').write_text(json.dumps(rows,indent=2)+'\n')
    print(json.dumps([{k:r.get(k) for k in ['run_dir','status','drops','gate_mission_s','third_release_s','search_seconds','search_xy_m','lane_spacing','fresh_fraction_of_accepted','max_active_seen_estimate_m2']} for r in rows],indent=2))
