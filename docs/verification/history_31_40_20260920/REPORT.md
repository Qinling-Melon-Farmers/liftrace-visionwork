...
> **2026-09-23 后续验收入口**：原始十轮的4/10和23/30保持冻结，不以新轮替换。先前失败六seed版本在31/32/37通过、34/38/40失败；本次仅对34/38/40采用更新后的线索延期版，三组主结果均FAIL，其中seed40按用户要求同版本重跑一次并以第二次进入主表。三组的[详细分析、全航迹、高度/速度、录像](../failed_three_20260923/REPORT.md)和[十seed跨版本对照](TEN_SEED_FOLLOWUP_20260923.md)附后。后续改动（局部近墙复核、遗漏航带优先和10cm水平膨胀）尚无SITL验证。
