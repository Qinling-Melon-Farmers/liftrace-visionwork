...
sources=[('R64',V/'r64_matrix/flight_metrics.json'),('full_random_31_35',V/'full_random_five_20260910/flight_metrics.json'),('paired_32_34',V/'high_view_full_20260914/summary.json'),('previous2672',V/'reliability_trial_20260919/metrics.json'),('currentABC',D/'summary.json')]
...
for c in classes:
    rows=[r for r in samples if r['class_name']==c]
...
axes[1].set(xlabel='FC - target X (cm)',ylabel='FC - target Y (cm)',title='Directions belong to each run frame; radii are comparable');axes[1].set_aspect('equal');axes[1].legend(fontsize=8)
