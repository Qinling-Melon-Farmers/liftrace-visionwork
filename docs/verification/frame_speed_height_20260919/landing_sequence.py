...
for item in json.loads((D/'runs.json').read_text()):
    ...
    land=next(e['ros_sec'] for e in events if e['kind']=='decision' and e['data']['command']==5)
    ...
    row['contact_events']=json.loads((run/'gazebo_contact_status.json').read_text()).get('events',[])
