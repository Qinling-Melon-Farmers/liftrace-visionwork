...
def one(item):
    ...
    axes[0].plot(a[:,0]-start,np.where(region,limit,np.nan),'r--',label='Corridor gate limit (spatial)')
...
def main():
    ...
    for ax,key,title in zip(axes,['top3_interrupt_mission_s','third_commit_s','completed_mission_s'],['Top3 interruption (s)','Third delivery ACK (s)','Completed mission (s)']):
        for i,m in enumerate(metrics):
            ...
            else:ax.text(i,0,'Not completed',rotation=90,ha='center')
    ...
    for phase in ['start_to_third_ack','last_release_recovery','fast_transfer_to_staging','staging_descent','corridor_to_H','H_landing']:
        values=[m['milestone_durations_s'][phase] or 0 for m in metrics];ax.barh(range(len(metrics)),values,left=left,label=phase);left+=values
