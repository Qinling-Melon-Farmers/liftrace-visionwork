...
def main():
    ...
    if state['status']!='COMPLETE' or [v['seed'] for v in state['results']]!=[31,32,34,37,38,40]:raise RuntimeError('Wait for all six runs')
    ...
    for row in state['results']:
        ...
        if not output.exists():
            composer=D/'compose_review.py' if seed in (38,40) else R/'vision_ws/src/uav_high_view/scripts/presentation_compose.py'
    ...
    fig,axes=plt.subplots(1,2,figsize=(13,5));xs=np.arange(6)
    ...
    lines=['# 历史失败六seed统一回归报告\n',f'本批仅重跑历史失败的31、32、34、37、38、40，各一次。结果为 **{ok}/6 PASS**。原十轮其他4个通过seed未复测，不能将此推算为当前版本十轮成功率。\n',
      '## 版本、场地和计时\n',f'统一HEAD `{state["source"]}`，实际导航/视觉包路径均核对为本工作树。六轮未调参、未补跑替换失败。与原十轮直接比对实际生成的五类靶位，最大XY差均小于0.1mm；树/门复用同一冻结world。2.6m高位、既有快走廊和录像配置一致。宿主墙钟改为6000秒，ROS任务600秒不变，避免旧seed40墙钟截尾；因此该项属于评测环境变化。\n',
    ...
    (D/'index.html').write_text('<!doctype html><html lang="zh-CN"><meta charset="utf-8"><title>历史失败六seed回归</title><style>body{max-width:1400px;margin:auto;font:16px sans-serif;padding:24px;background:#f7f8fa;color:#18212b}video{width:100%;max-width:960px}.plots{display:grid;grid-template-columns:repeat(2,1fr);gap:12px}img{width:100%}section{margin:32px 0;padding:20px;background:white}</style><h1>历史失败六seed回归</h1><p><a href="REPORT.md">统一报告</a> · <a href="comparison.json">配对数据</a></p>'+''.join(sections)+'</html>')
    (D/'validation.json').write_text(json.dumps(dict(six_complete=True,source_consistent=True,actual_targets_matched=True,all_videos_decoded=True),indent=2)+'\n')
