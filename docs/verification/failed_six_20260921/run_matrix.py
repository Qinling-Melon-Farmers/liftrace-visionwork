"""Explicitly authorized frozen six-seed regression. No retries or tuning."""
from pathlib import Path
import argparse,collections,fcntl,json,os,signal,subprocess,time
D=Path(__file__).resolve().parent;R=D.parents[2]
SEEDS=(31,32,34,37,38,40)

def main():
    p=argparse.ArgumentParser();p.add_argument('--execute-authorized-six',action='store_true');args=p.parse_args()
    if not args.execute_authorized_six:raise SystemExit('Explicit authorization flag required')
    lock=open('/tmp/liftrace_failed_six_20260921.lock','w');fcntl.flock(lock,fcntl.LOCK_EX|fcntl.LOCK_NB)
    batch=R/'logs/failed_six_20260921_batch';batch.mkdir(exist_ok=True);statefile=batch/'matrix.json'
    ...
    for seed in SEEDS:
        ...
        prefix=f'failedsix_seed{seed}';before=set((R/'logs').glob(prefix+'_*'))
    ...
    state['status']='COMPLETE';save();print('Six finished; no retries',flush=True)
