...
PHASES={'IDLE':'待启动','STARTUP':'系统准备','SEARCH':'搜索','SURVEY':'高位快速搜索','ASCEND':'起飞 / 升高','DESCEND':'回降低空',
        ...
        'POST_DELIVERY_ROUTE':'走廊穿行','LAND':'H标志降落','COMPLETE':'任务完成','ABORT':'任务中止'}
...
def main():
    ...
    contact_path=source/'gazebo_contact_status.json'
    contacts=json.loads(contact_path.read_text()).get('events',[]) if contact_path.exists() else []
    guard_only=bool(contacts) and all(any('competition_guard_collision' in part for part in pair)
        for event in contacts for pair in event.get('pairs',[]))
    ...
    try:
        for t in np.arange(first,last+1e-8,.1):
            ...
            # Mission transactions supersede the long-lived LOW_COVERAGE mode.
            if state.get('active_command')=='APPROACH':phase='DELIVERY'
            if state.get('active_command')=='RETURN_HOME':phase='POST_DELIVERY_ROUTE'
            if state.get('phase') in ('LAND','COMPLETE','ABORT'):phase=state['phase']
            ...
            if gate.get('reason')=='actual_collision' and guard_only:
                explanation='鲁棒包络接触（见报告）'
