"""One focused seed40 rerun after the shared yaw-budget correction."""
...
def main():
    ...
    lock=open('/tmp/liftrace_near_wall_degrade_20260923.lock','w');fcntl.flock(lock,fcntl.LOCK_EX|fcntl.LOCK_NB)
    batch=R/'logs/near_wall_degrade_20260923_seed40_yaw_budget_batch';batch.mkdir(exist_ok=True);statefile=batch/'matrix.json'
    ...
    for seed in SEEDS:
        ...
        prefix=f'nearwall_yawbudget_seed{seed}';before=set((R/'logs').glob(prefix+'_*'))
        ...
        row['first_failure_analysis']=dict(terminal=gate['reason'],errors=gate.get('errors',[]),action_failures=failures,scope='Focused new-source validation; original result retained')
    ...
    state['status']='COMPLETE';save();print('Focused seed40 finished; no retries',flush=True)
