...
2026-09-12固定树增补：按用户要求完成R64固定树箱、随机靶/门四轮同图对照，3 PASS、1次LAND故障。两个布局入口开启后搜索耗时均增加；seed32小幅缩短航程仍被低速开销抵消。原型保持默认关闭，下一步先完善无插点收益基准和实际动作成本。[本批报告与18张图](../../verification/fixed_tree_efficiency_20260912/REPORT.md)，优先级以ROADMAP最新条目为准。

2026-09-12实跑增补：四轮同图局部执行关/开已完成，seed34执行组37/37 PASS，其余3组末段碰撞；两个布局搜索耗时和航程都增加。当前原型默认关闭，先改收益/成本对照、入口触发和额度分配，再扩大验证。整节点CPU目标尚未达到；正赛与交付保持。[完整结果及图表](../../verification/local_entry_sitl_20260912/REPORT.md)，后续优先级以ROADMAP最新条目为准。下方保留启动前阶段记录。

2026-09-12执行原型增补：局部入口已能经原任务链接纳，默认关闭；每名义点一次/每场四次/单次12s，原deadline保留，入口到达不完成名义航点，投递仍优先。本阶段收益按entry观察区域计算，不预执行exit。两套构建与391项相关测试、构造执行器回放通过；新SITL仍未运行。见[执行与存储报告](../../verification/local_entry_execution_20260912/REPORT.md)，下一步实跑顺序及指标以ROADMAP为准。
