...
2026-09-15后续执行：[理想视场、三轮重跑及32补验报告](../../verification/high_fallback_repair_20260915/REPORT.md)。原三轮整场0/3、三投2/3，32补验两投后碰墙；保留a12750b下降解耦及持久导航线索、有界换点、低位覆盖兜底，183项回归通过。下一优先级见ROADMAP，原设计历史保留。

2026-09-15设计补充：[高位可达性、seed32线索失效与seed31缺靶兜底](FALLBACK_20260915.md)。采用持久导航线索、有限横向航点替代、部分目标先复访及低位补搜；当前仅设计，未修改飞行程序或追加实跑。

2026-09-14执行补充：[单目标动态复访](../../verification/high_view_dynamic_probe_20260913/REPORT.md)已在两个布局完成，含ID=0接口修复及一次同seed重跑，三次分段PASS。当前未执行投递或整场优化；下一步先改善高位可见目标集合和返回成本，再接原释放/恢复链。

2026-09-13执行补充：[两布局四高度Gazebo筛查](../../verification/high_view_render_20260913/REPORT.md)已完成480帧。用户确认高位坐标用于低位粗复访，本批约0.12–0.16m理想位姿投影误差不单独阻断下一步；携带误差范围、低位重新捕获、原规划净空与释放许可仍必须成立。新增优先验证重捕率与净节时，投影一致性并行改善。该批不是飞行/动态定位或P0全部验收。
...
## 2026-09-14 用户确认后的完整任务试验口径
本节覆盖前文45s受限探针预算和第一轮仅单目标的旧执行安排；历史几何与风险分析继续保留。
当前已实现完整高位闭合航线，累计三类高权重唯一稳定坐标后中断；不以45s结束。回降后按感知栅格成本选择三点访问顺序（含最后目标至走廊），新鲜低位识别再接原投递、恢复、过门、H降落。此版本不遍历低位牛耕线；航线结束缺高权重目标会明确失败。
本轮已授权两组对照、总共四次：固定树seed32/34，每seed各一次原低位策略和高位完整策略；同地图、模型与原整场Gate。原低位巡航为FC AGL1.4m，0.70m指搜索航带间距，不能当巡航高度。
每轮保存完整航迹、指令航迹、导航点、高度、速度、目标事务与阶段时间图；配对汇总第三次有效投递和最终降落时间差。失败一并列出。详见../../verification/high_view_full_20260914/PLAN.md，结论待四轮结束。

### 最新执行修订：以策略净收益为核心
按用户反馈，完整方案调整为中断位置立即排序并优先就地下降，附近下降点仅作受阻替代；高位导航线索门槛适度放宽，低位释放标准不变。原回起飞柱版本作为开发历史，不作为最终推荐。停止继续扩展功能，冻结后完成两seed四轮配对，以第三投和整场净收益、目标价值及失败情况作结论。参数、开发失败和图表见../../verification/high_view_full_20260914/PLAN.md。
