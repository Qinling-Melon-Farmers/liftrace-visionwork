...
用户本轮确认后续恢复无头策略。当前正式入口本就默认`gui=false`、`rviz=false`，无需改飞行算法或重跑；无头保留服务端机载图像渲染及关键日志；按用户最新要求，后续关闭所有录像。[以本机AstraDroneOpen为来源的裸机配置方案](BARE_METAL_HEADLESS.md)。
...
先单作业复现seed11，检查图像非黑屏、内外参与本地一致、LIO/规划/任务链时钟正常、日志完整及收尾无残留，再试2、4作业逐级测量。保持物理步长、传感器频率、控制器频率和600s仿真任务时限；不能为跑得快降低感知频率或关掉相机后沿用同一PASS口径。墙钟watchdog应按实际RTF留足，而非用600墙钟秒杀掉尚未达到600仿真秒的慢作业。
...
按用户最新要求，后续矩阵只留txt/json/jsonl/csv、Gate、参数快照及飞控诊断日志，不录机载/俯视/桌面视频，也不录全场bag。视觉仍在线消费机载图像；按首轮日志量设磁盘配额。板端RKNN并发与实机降落问题仍需在目标硬件验收，远程SITL不能代替。
