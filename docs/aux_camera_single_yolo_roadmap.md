...
2026-08-20 已完成三轮无 GUI 对比，聚合目录为
`logs/oblique_guided_search_ab_55deg_20260820_170138/`：

* 原 12 点覆盖与 guided 均为 3/3 PASS、五类全部发现、零释放；
* 基线搜索中位数 174.402 s、路径 34.304 m；
* guided 搜索中位数 135.800 s、路径 22.769 m；
* 三组配对节省中位数 36.000 s（21.0%）、路径 11.913 m；
* 但三轮辅助激活率均为 0，所有匿名蓝环只入队，收益只能归因于四点稀疏覆盖；
* 强制 2 候选轮已有 2/2 交接证据，但搜索 316.499 s、路径 45.124 m，说明匿名候选过早
  中断在当前固定世界没有收益。

因此 Step 1 按以下口径收口：候选访问与下视交接机制成立，默认业务策略不会破坏任务闭环；
固定坐标世界没有证明辅助相机带来收益。后续不再用这 21% 宣称辅助相机提速。

下一 Gate 进入 Step 2 `Aux Proposal Provider`，统一 OpenCV/YOLO 粗候选来源。完成来源抽象
和零控制输出 L0 后，再加入辅助 YOLO 解决红十字/语义高权重中断；单 YOLO 双输入与全随机
世界仍按原顺序后置。
