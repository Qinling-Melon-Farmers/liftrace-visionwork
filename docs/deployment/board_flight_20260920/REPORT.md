...
9月21日新增：已核对试飞组提供的22点无障碍轮和23点第二门停滞轮源码、日志及点云bag，见[新实飞分析与四轮改动复查](../flight_review_20260921/REPORT.md)。该现场入口仍为旧轨迹执行链，不能当作新FSM修复后的飞行结果。
...
本次异常的日志、实机/仿真参数差异和分层修复建议见[地图占据ABORT专项分析](MAP_ABORT_ANALYSIS.md)；FreeDOM的积分、清除、点龄缺口及下一轮最小诊断集见[FreeDOM静态点云专项分析](FREEDOM_MAP_ANALYSIS.md)。当前只能确认终点占据，因失败时未保存静态点云/膨胀地图，尚不能区分实体障碍、水平柱误分类、静态点残留或单位TF偏差。

后续已将FSM显式保持、全阶段首轨迹期限、A*起步原语/拒绝诊断及高位低空staging同步到本板端试飞分支；状态和边界见[板端可靠性同步说明](RELIABILITY_SYNC.md)。该代码尚未重新部署或实飞。
