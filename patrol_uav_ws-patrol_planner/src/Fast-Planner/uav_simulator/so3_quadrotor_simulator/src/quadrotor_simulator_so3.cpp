...
main(int argc, char** argv)
{
  ...
  */
  ...
  while (n.ok())
  {
    ...
    Disturbance dist_snapshot;
    {
      std::lock_guard<std::mutex> lock_cmd(cmd_mutex);
      cmd_snapshot = command;
    }
    {
      std::lock_guard<std::mutex> lock_dist(dist_mutex);
      dist_snapshot = disturbance;
    }

    auto last = control;
    control   = getControl(quad, cmd_snapshot);
    for (int i = 0; i < 4; ++i)
    {
    ...
    quad.setInput(control.rpm[0], control.rpm[1], control.rpm[2],
                  control.rpm[3]);
    quad.setExternalForce(dist_snapshot.f);
    quad.setExternalMoment(dist_snapshot.m);
    quad.step(dt);
